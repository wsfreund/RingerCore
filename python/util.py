...
class EnumStringification:
  ...
  def retrieve(cls, val):
    ...
    """
    allowedValues = get_attributes(cls)
    if type(val) is str:                                                                    
...
class Roc(object):
  ...
  def __init__( self, output, target, label, numPts = 1001, npConst = None ):
    """
			def ROC( output, target, label, numPts = 1001, npConst = npConstants() ):
    ...
    if npConst is None: npConst = npConstants()
    self.label = label
    outSignal = output[np.where(target == 1.)[1]]
    outSignal = np.sort(outSignal, kind='heapsort')
    outNoise = output[np.where(target == -1.)[1]]
    outNoise  = np.sort(outNoise , kind='heapsort')
    self.cutVec = np.arange( -1., 1., 2. / ( numPts - 1 ) )
    self.detVec = npConst.fp_zeros( len(self.cutVec) )
    self.faVec  = npConst.fp_zeros( len(self.cutVec) )
    self.spVec  = npConst.fp_zeros( len(self.cutVec) )
    lenSig   = float( len(outSignal) )
    lenNoise = float( len(outNoise)  )
    for i in range(len(self.cutVec)):
